...
public class EmployeesReadRepository : Domain.Repositories.Employees.IEmpoyeesReadRepository
{
    ...
    #region Dependencies
    readonly Abstractions.ModelExtractor<Models.CAMPING_AI_DB.T_EMPLOYEES> _modelExtractor;
    ...
    public EmployeesReadRepository(Abstractions.ModelExtractor<Models.CAMPING_AI_DB.T_EMPLOYEES> modelExtractor,
    ...
    public async Task<Domain.Entities.Employee?> Get(Guid id, bool onlyNotDeleted)
    {
        ...
        sql.AppendLine($"WHERE {nameof(Models.CAMPING_AI_DB.T_EMPLOYEES.EMP_IdEmployee)} = @IdEmployee ");
        if (onlyNotDeleted)
        {
            sql.AppendLine($"AND {nameof(Models.CAMPING_AI_DB.T_EMPLOYEES.EMP_DeletedOn)} IS NULL ");
        }
        sql.AppendLine($"order by {nameof(Models.CAMPING_AI_DB.T_EMPLOYEES.EMP_CreatedOn)} DESC");
        string query = sql.ToString();
        Models.CAMPING_AI_DB.T_EMPLOYEES? modelDB;
        try
        {
            modelDB = await dbConnection.QueryFirstOrDefaultAsync<Models.CAMPING_AI_DB.T_EMPLOYEES>(query, new { IdEmployee = id.ToString() });
        }
        catch (Exception ex)
        {
    ...
    public async Task<IEnumerable<Domain.Entities.Employee>> GetAllAsync(bool onlyNotDeleted)
    {
        ...
        if (onlyNotDeleted)
        {
            sql.AppendLine($"WHERE {nameof(Models.CAMPING_AI_DB.T_EMPLOYEES.EMP_DeletedOn)} IS NULL");
        }
        sql.AppendLine($"order by {nameof(Models.CAMPING_AI_DB.T_EMPLOYEES.EMP_CreatedOn)} DESC");
        string query = sql.ToString();
        IEnumerable<Models.CAMPING_AI_DB.T_EMPLOYEES> modelsDB;
        try
        {
            modelsDB = await dbConnection.QueryAsync<Models.CAMPING_AI_DB.T_EMPLOYEES>(query);
        }
        catch (Exception ex)
        {
